...
from models.transaction import save_transaction, get_additional_services, verify_save  
        ...
        if st.button("Complete Transaction", type="primary", use_container_width=True):
            ...
            if save_transaction(transaction_data):
                try:
                    ...
                    verification = verify_save(transaction_data['service_id'])
                    if verification.get('STATUS') != 'COMPLETED':
                        st.error("Transaction status not updated properly. Please try again.")
                        return False
                    ...
                    print(f"Transaction {transaction_data['service_id']} completed successfully with status: {verification.get('STATUS')}")
                    print(f"Completion date: {verification.get('COMPLETION_DATE')}")
                    
                    st.session_state['page'] = 'completed_services'
                    st.rerun()
                    
                except Exception as e:
                    print(f"Error in completion process: {str(e)}")
