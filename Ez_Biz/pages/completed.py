...
def display_payment_form(row: pd.Series) -> bool:
    ...
    Args:
        row (pd.Series): Row of payment data
    """
...
def send_payment_reminder(data: dict) -> bool:
    ...
    try:
        message = f"""
        Payment Reminder - EZ Biz Services
        
        Dear {data['customer_name']},
        
        This is a reminder about your outstanding balance for services provided on {data['service_date']}.
        
        Amount Due: ${data['balance_due']:.2f}
        ...
        if data['preferred_contact'] == "Email" and data['email_address']:
            # TODO: Implement actual email sending
            st.info(f"Email reminder would be sent to: {data['email_address']}\n{message}")
            return True
            
        elif data['preferred_contact'] in ["Phone", "Text"] and data['phone_number']:
        ...
        with col1:
            if st.button("Send Reminder", key=f"send_reminder_{row['TRANSACTION_ID']}"):
                reminder_data = {
                    ...
                    'preferred_contact': reminder_type.upper()
                }
                if send_payment_reminder(reminder_data):
                    st.success("Payment reminder sent successfully!")
        
        with col2:
            if st.button("Send Invoice", key=f"send_invoice_{row['TRANSACTION_ID']}"):
                invoice_data = {
                    'customer_name': row['CUSTOMER_NAME'],
                    'service_date': row['TRANSACTION_DATE'].strftime('%Y-%m-%d'),
...
def completed_services_page():
        ...
        with col2:
            if payment_status == "Unpaid":
                send_reminders = st.checkbox(
                    ...
                    help="Allow sending reminders to all unpaid services at once"
                )
    ...
    LEFT JOIN OPERATIONAL.CARPET.EMPLOYEE E1 ON ST.EMPLOYEE1_ID = E1.EMPLOYEE_ID
    LEFT JOIN OPERATIONAL.CARPET.EMPLOYEE E2 ON ST.EMPLOYEE2_ID = E2.EMPLOYEE_ID
    LEFT JOIN OPERATIONAL.CARPET.EMPLOYEE E3 ON ST.EMPLOYEE3_ID = E3.EMPLOYEE_ID
    WHERE ST.TRANSACTION_DATE BETWEEN ? AND ?
    AND ST.END_TIME IS NOT NULL  -- Only show completed services
    ...
    if results:
        ...
        # Bulk Reminder Option for Unpaid Services
        if payment_status == "Unpaid" and send_reminders and not completed_df.empty:
            st.markdown("### Bulk Payment Reminders")
            bulk_col1, bulk_col2 = st.columns(2)
            with bulk_col1:
                bulk_reminder_type = st.selectbox(
                    "Reminder Method",
                    ["Email", "SMS", "Both"],
                    key="bulk_reminder_type"
                )
            with bulk_col2:
                    ...
                    with st.spinner("Sending payment reminders..."):
                        ...
                        for _, row in completed_df.iterrows():
                            ...
                            reminder_data = {
                                ...
                                'preferred_contact': bulk_reminder_type.upper()
                            }
                            if send_payment_reminder(reminder_data):
                                success_count += 1
                        
                        st.success(f"Successfully sent {success_count} payment reminders")
        ...
        for _, row in completed_df.iterrows():
            with st.expander(f"{row['TRANSACTION_DATE'].strftime('%Y-%m-%d')} - {row['CUSTOMER_NAME']}"):
                ...
                st.write("### Payment Actions")
                action_col1, action_col2, action_col3 = st.columns(3)
                
                # Invoice Generation (Always Available)
                with action_col1:
                ...
                # Payment Reminder (Only for Unpaid)
                with action_col2:
...
#     """
#     result = snowflake_conn.execute_query(debug_query)
#     st.write(f"Total transactions in database: {result[0]['COUNT']}")
...
#         ST.PYMT_MTHD_1,
#         ST.PYMT_MTHD_2,
#         CAST(COALESCE(ST.PYMT_MTHD_1_AMT, 0) AS FLOAT) AS PYMT_MTHD_1_AMT,
#         CAST(COALESCE(ST.PYMT_MTHD_2_AMT, 0) AS FLOAT) AS PYMT_MTHD_2_AMT,
#         ST.DEPOSIT_PAYMENT_METHOD,
#         C.EMAIL_ADDRESS,
#         C.PHONE_NUMBER,
#         ST.COMMENTS,
...
#     if results:
#         st.write(f"Found {len(results)} completed transactions")
#         completed_df = pd.DataFrame(results)
