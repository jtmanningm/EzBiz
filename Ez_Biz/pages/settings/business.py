...
#             try:
#                 # Debug information
#                 st.write("Form submitted with values:")
#                 st.write({
...
#                     'email': email,
#                     'website': website,
#                     'weekday_start': weekday_start.strftime('%H:%M'),
#                     'weekday_end': weekday_end.strftime('%H:%M'),
#                     'weekend_start': weekend_start.strftime('%H:%M'),
#                     'weekend_end': weekend_end.strftime('%H:%M')
#                 })
...
#                     ]
#                     st.write("Executing UPDATE query")
#                 else:
#                     query = """
...
#                         weekend_end.strftime('%H:%M')
#                     ]
#                     st.write("Executing INSERT query")
...
def business_settings_page():
    ...
    try:
        ...
        st.error(f"Failed to fetch current settings: {str(e)}")
    ...
    if current_settings:
        try:
            ...
            try:
                ...
                st.warning("Failed to parse current settings.")
                st.write("Full error details:", traceback.format_exc())
    ...
    with st.form("business_settings_form"):
        ...
        # Input fields
        business_name = st.text_input("Business Name", value=str(settings.get('BUSINESS_NAME', '')).strip())
        phone = st.text_input("Business Phone", value=str(settings.get('PHONE_NUMBER', '')).strip())
        email = st.text_input("Business Email", value=str(settings.get('EMAIL_ADDRESS', '')).strip())
        website = st.text_input("Website", value=str(settings.get('WEBSITE', '')).strip())

        # Address Information
        st.header("Business Address")
        street_address = st.text_input("Street Address", value=str(settings.get('STREET_ADDRESS', '')).strip())
        city = st.text_input("City", value=str(settings.get('CITY', '')).strip())
        state = st.text_input("State", value=str(settings.get('STATE', '')).strip())
        zip_code = st.text_input("ZIP Code", value=str(settings.get('ZIP_CODE', '')).strip())

        # Operating Hours
        st.header("Operating Hours")

        # Convert time values explicitly
        ...
        weekday_start = st.time_input("Weekday Opening Time", value=parse_time(settings.get('OPERATING_HOURS_START'), time(8, 0)))
        weekday_end = st.time_input("Weekday Closing Time", value=parse_time(settings.get('OPERATING_HOURS_END'), time(17, 0)))
        weekend_start = st.time_input("Weekend Opening Time", value=parse_time(settings.get('WEEKEND_OPERATING_HOURS_START'), time(9, 0)))
        weekend_end = st.time_input("Weekend Closing Time", value=parse_time(settings.get('WEEKEND_OPERATING_HOURS_END'), time(15, 0)))
        ...
        if submitted:
            try:
                ...
                zip_code_value = int(zip_code) if zip_code.strip().isdigit() else None
                if settings.get('BUSINESS_ID'):
                    query = """
                    UPDATE OPERATIONAL.CARPET.BUSINESS_INFO
                    SET BUSINESS_NAME = ?,
                    ...
                    params = [
                        ...
                        zip_code_value, phone, email, website,
                        weekday_start.strftime('%H:%M'), weekday_end.strftime('%H:%M'),
                        weekend_start.strftime('%H:%M'), weekend_end.strftime('%H:%M'),
                        settings['BUSINESS_ID']
                    ]
                else:
                    query = """
                    INSERT INTO OPERATIONAL.CARPET.BUSINESS_INFO (
                        ...
                        PHONE_NUMBER, EMAIL_ADDRESS, WEBSITE, OPERATING_HOURS_START, OPERATING_HOURS_END,
                        WEEKEND_OPERATING_HOURS_START, WEEKEND_OPERATING_HOURS_END, ACTIVE_STATUS
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, TRUE)
                    """
                    params = [
                        business_name, street_address, city, state,
