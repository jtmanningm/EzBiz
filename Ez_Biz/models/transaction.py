...
def save_transaction(transaction_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        params = [
            int(transaction_data['customer_id']) if transaction_data.get('customer_id') else None,
            int(transaction_data['account_id']) if transaction_data.get('account_id') else None,
            float(transaction_data['final_amount']),
            float(transaction_data.get('discount', 0)),
            float(transaction_data['deposit']),
            float(transaction_data['amount_received']),
            str(transaction_data['payment_method_1']) if transaction_data.get('payment_method_1') else None,
            float(transaction_data['payment_amount_1']),
            str(transaction_data['payment_method_2']) if transaction_data.get('payment_method_2') else None,
            float(transaction_data['payment_amount_2']),
            str(transaction_data.get('deposit_payment_method')) if transaction_data.get('deposit_payment_method') else None,
            int(transaction_data['employee1_id']) if transaction_data.get('employee1_id') else None,
            int(transaction_data['employee2_id']) if transaction_data.get('employee2_id') else None,
            int(transaction_data['employee3_id']) if transaction_data.get('employee3_id') else None,
            ...
            str(price_adjustments_json),
            int(transaction_data['service_id'])
        ]

        print("Executing transaction update with params:", params)
        snowflake_conn = SnowflakeConnection.get_instance()
        snowflake_conn.execute_query(query, params)

        # Update upcoming service status
        ...
        print(f"Updating service {transaction_data['service_id']} to completed status")
        snowflake_conn.execute_query(service_update_query, [int(transaction_data['service_id'])])
        
        print("Successfully saved transaction and updated service status")
        return True
...
def verify_transaction_status(service_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        snowflake_conn = SnowflakeConnection.get_instance()
        result = snowflake_conn.execute_query(query, [int(service_id)])
        
        if result and len(result) > 0:
            row = result[0]
            if hasattr(row, '_asdict'):
                status_info = row._asdict()
            else:
                status_info = dict(row)
                
            print(f"Service ID {service_id} status:")
            print(f"Transaction Status: {status_info['TRANSACTION_STATUS']}")
            print(f"Service Status: {status_info['SERVICE_STATUS']}")
            print(f"Completion Date: {status_info['COMPLETION_DATE']}")
            return status_info
        else:
