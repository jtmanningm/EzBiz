...
def save_service_schedule(
    ...
    try:
        ...
        price_adjustments_str = json.dumps(price_adjustments)
        ...
        transaction_params = [
            ...
            0.0,        # DISCOUNT
            0.0,        # AMOUNT_RECEIVED
            float(deposit_amount),  # DEPOSIT
            None,       # DEPOSIT_PAYMENT_METHOD
            notes,      # COMMENTS
            base_cost,  # BASE_SERVICE_COST
            0.0,        # MATERIAL_COST
            0.0,        # TOTAL_LABOR_COST
            0.0,        # MARKUP_PERCENTAGE
            price_adjustments_str  # PRICE_ADJUSTMENTS_JSON
        ]
        ...
        upcoming_params = [
            ...
            False  # DEPOSIT_PAID defaults to false
        ]
        
        # Execute upcoming service insert
        snowflake_conn.execute_query(upcoming_service_query, upcoming_params)
...
def schedule_recurring_services(
    ...
    try:
        ...
        while current_date < one_year_from_now:
            if recurrence_pattern == "Weekly":
                ...
                try:
                    ...
                    # If the day doesn't exist in target month (e.g., 31st), use last day
                    if month + 1 > 12:
        ...
        print(f"Scheduling {len(future_dates)} recurring services up to {one_year_from_now.strftime('%Y-%m-%d')}")
